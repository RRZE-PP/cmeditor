<div id="cmEditorMenu" class="cmeditor-menu">
	...
	<ul class="cmeditor-menubar menu" id="asdfasdfasdf">
		<li><a href="#">File</a>
			<ul class="fileMenu">
...
//TODO: sollten die haken zb bei diffbeforesafe nicht erstmal das korrekte darstellen?
//TODO: bei readOnly mehr sachen ausgrauen
//TODO: Einmal drueber grepen und alle TODOS fixen
//TODO: Einmal drueber grepen und alle ${} aufloesen
	this.CMEditorMenu = (function(){

		function CMEditorMenu(cmeditor, rootElem, options){
			this.cmeditor = cmeditor;
			this.rootElem = rootElem = $(rootElem);
		...
		function initMenus(self){
			...
			self.fileMenu = {
				...
				rename: function(cm) {
					...
					if (name == null) return;
					if (!name) name = "test";
					console.log(self.cmeditor.get_name(name));
		...
		function init(self, options) {
			...
			if(self.options.addModes){
				for (var i = self.options.addModes.length - 1; i >= 0; i--) {
					...
					s.appendTo(self.rootElem.find(".modes"));
			...
			console.log("menubar initiated")

			self.rootElem.find('.fileMenu a').click(function(event) {
				console.log('file menu entry clicked');
			...
			self.rootElem.find('.viewMenu a').click(function(event) {
			    ...
			    if ($(this).attr('value').indexOf('readOnly') == 0) {
			    	if (self.cmeditor.getOption('readOnly')) {
			    		$(this).children('span').addClass('ui-icon ui-icon-check');
			    	} else {
			...
			self.rootElem.find('.optionsMenu a').click(function(event) {
				...
				//fixme options.useSession kommt aus grails
			...
			self.rootElem.find(".modes a[value='mode"+self.options.mode+"']").children('span').addClass('ui-icon ui-icon-check');

			if(self.options.useSession){
				if (localStorage['cmeditor-menu-binding'])
					self.rootElem.find(".options a[value='binding"+localStorage['cmeditor-menu-binding']+"']").children('span').addClass('ui-icon ui-icon-check');
				else
					self.rootElem.find(".options a[value='bindingdefault']").children('span').addClass('ui-icon ui-icon-check');

				if (localStorage['cmeditor-menu-theme'])
					self.rootElem.find(".options a[value='theme"+localStorage['cmeditor-menu-theme']+"']").children('span').addClass('ui-icon ui-icon-check');
				else
					self.rootElem.find(".options a[value='themedefault']").children('span').addClass('ui-icon ui-icon-check');

				if (localStorage['cmeditor-menu-diffBeforeSave'])
					self.rootElem.find(".options a[value='diffBeforeSave']").children('span').addClass('ui-icon ui-icon-check');
				else if (self.options.defaultDiffBeforeSave)
					self.rootElem.find(".options a[value='diffBeforeSave']").children('span').addClass('ui-icon ui-icon-check');

			}else{
				self.rootElem.find(".options a[value='binding"+options.binding+"']").children('span').addClass('ui-icon ui-icon-check');
				self.rootElem.find(".options a[value='theme"+options.theme+"']").children('span').addClass('ui-icon ui-icon-check');
			...
			if(self.options.defaultReadOnly){
				self.rootElem.find(".view a[value='readOnly']").children('span').addClass('ui-icon ui-icon-check');
			}
			if(self.options.readOnly){
				self.rootElem.find(".view a[value='readOnly']").children('span').addClass('ui-icon ui-icon-check');
				self.rootElem.find(".view a[value='readOnly']").parent().addClass('ui-state-disabled');
				self.rootElem.find(".file a[value='new']").parent().addClass('ui-state-disabled');
				self.rootElem.find(".file a[value='save']").parent().addClass('ui-state-disabled');
				self.rootElem.find(".file a[value='saveas']").parent().addClass('ui-state-disabled');
				self.rootElem.find(".file a[value='rename']").parent().addClass('ui-state-disabled');
				self.rootElem.find(".file a[value='delete']").parent().addClass('ui-state-disabled');
			}

			if(typeof self.cmeditor.diff != 'function') {
				self.rootElem.find(".view a[value='diff']").parent().remove();
				self.rootElem.find(".options a[value='diffBeforeSave']").parent().remove();
			}

			self.cmeditor.focus();
			console.log("cmeditor_menu loaded.");
		}

		function update(self) {
			self.rootElem.find('.modes').find('span').removeClass('ui-icon ui-icon-check');
			self.rootElem.find(".modes a[value='mode"+self.cmeditor.get_mode()+"']").children('span').addClass('ui-icon ui-icon-check');
			...
			console.log("cmeditor_menu_update was performed.")
		}
